...
C
C     Robust fix for gfortran: never READ-to-EOF to position file.
C     Instead, reopen with POSITION='APPEND' for writing.
C--------------------------------------------------------------
      INCLUDE 'XFOIL.INC'
...
C---- open ONLY to read header and check consistency
        OPEN(LU,FILE=FNAME,STATUS='OLD',FORM='UNFORMATTED',
     &       ACTION='READ',ERR=80)

        READ(LU,ERR=90,END=90) NAMEX
        READ(LU,ERR=90,END=90) MACHX, REYNX, ACRITX
        READ(LU,ERR=90,END=90) MATYPX, RETYPX
        READ(LU,ERR=90,END=90) IIX, ILEX, ITEX, IIBX
        CLOSE(LU)
...
C---- check if dump file is for the same airfoil and conditions
        IF(NAMDIF                    .OR.
     &     REYNX  .NE. REINF1   .OR.
     &     MACHX  .NE. MINF1    .OR.
     &     ACRITX .NE. ACRIT    .OR.
...
C---- reopen for appending (NO EOF state possible here)
        OPEN(LU,FILE=FNAME,STATUS='OLD',FORM='UNFORMATTED',
     &       POSITION='APPEND',ACTION='WRITE',ERR=80)
      ...
      ELSE
C---- create new dump file header
        OPEN(LU,FILE=FNAME,STATUS='NEW',FORM='UNFORMATTED',
     &       ACTION='WRITE',ERR=80)
